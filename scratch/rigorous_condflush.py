"""VALUE: the production A/B that PICKS THE WINNER among checkpoint store modes —
fp16 RNE (current) vs fp16+SR (stochastic rounding, the SOTA in TRT-LLM/flashinfer/vLLM)
vs fp16+EF (error-feedback), all implemented in the real ReplaySSM kernel and selected
per-server via SGLANG_REPLAYSSM_STORE_MODE. recurrent = exact reference. CUDA graph OFF
on all 4 (fair: SR needs a per-call seed). Measures loop-frequency + accuracy over 60
AIME-24+25 problems x N repeats. Bring servers up with launch_storemode.sh first."""
...
N = 3
CONFIGS = {"recurrent": 31010, "base": 31011, "cf1.5": 31012, "cf2.5": 31013}
LOOP_TH, SEVERE_TH = 50, 200
MAXTOK = 32768
...
def ask(port, q):
    try:
        r = requests.post(f"http://127.0.0.1:{port}/v1/chat/completions", json={
            ...
            "temperature": 0, "max_tokens": MAXTOK}, timeout=7200)
        ...
        return {"tok": r.json().get("usage", {}).get("completion_tokens", 0),
                "maxrep": max_rep(full), "ans": (mm.group(1) if mm else None),
    ...
    for rep_i in range(N):
        ...
        with cf.ThreadPoolExecutor(max_workers=len(problems)) as ex:
            ...
            for f in cf.as_completed(futs):
                ...
                rows.append(rec)
        json.dump(rows, open(f"scratch/rigorous_condflush_{name}.json", "w"))
        print(f"[{name}] repeat {rep_i+1}/{N} done in {time.time()-t0:.0f}s ({len(rows)})", flush=True)
    return name, rows

def main():
    problems = load_problems()
    print(f"problems: {len(problems)}; modes: {list(CONFIGS)}; N={N}; max_tok={MAXTOK}; "
          f"cuda-graph OFF -> {len(problems)*N} samples/config", flush=True)
    out = {}
    ...
    json.dump(out, open("scratch/rigorous_condflush_all.json", "w"))
    print("\n========== COND-FLUSH A/B (winner = lowest loops + highest acc) ==========")
    print(f"{'mode':>10} | {'acc mean±std':>14} | {'loop>=50':>10} | {'loop>=200':>11} | {'trunc':>10} | {'avg_tok':>8}")
    for n in CONFIGS:
        ...
        print(f"{n:>10} | {accm:.3f}±{accs:.3f} | {l50:>3}/{ns:<5} | {l200:>3}/{ns:<6} | {tr:>3}/{ns:<5} | {at:>8}")
    print("raw -> scratch/rigorous_condflush_all.json")
