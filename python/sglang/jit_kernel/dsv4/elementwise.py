...
# (kRopeFirst=true), no Hadamard (kHadamard=false), MXFP4 output. is_neox
# selects the RoPE convention (kIsNeox), as in the FP8 rope-first module.
@cache_once
def _jit_main_q_indexer_rope_first_fp4_quant_module(dtype: torch.dtype, is_neox: bool):
    args = make_cpp_args(dtype, is_arch_support_pdl(), True, False, is_neox)
    return load_jit(
        make_name(f"main_q_indexer_rope_first_fp4_quant{'_neox' if is_neox else ''}"),
        *args,
        cuda_files=["deepseek_v4/main_norm_rope.cuh"],
        cuda_wrappers=[
            ("forward", f"FusedQIndexerRopeHadamardFp4QuantKernel<{args}>::forward"),
...
def fused_q_indexer_rope_first_fp4_quant(
    ...
    *,
    ...
    See fused_q_indexer_rope_first_quant for the is_neox convention.
    """
    if _is_hip:
    ...
    module = _jit_main_q_indexer_rope_first_fp4_quant_module(q_input.dtype, is_neox)
    module.forward(
        q_input,
        q_fp4,
        q_sf,
