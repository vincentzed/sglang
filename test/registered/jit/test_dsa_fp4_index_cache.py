...
# The fused K store shares its norm+rope path with fused_k_indexer_norm_rope
# and rounds through bf16 before quant, so it must match the un-fused
# bf16-kernel + store_fp4_index_k_cache path bit-for-bit. The Q kernel ropes
# in fp32 (fma scheduling may differ from the torch reference), so it is held
# to a high byte-match rate plus exact head-gate weights instead.
# ----------------------------------------------------------------------------
...
def _rope_first_ref(x, cos_p, sin_p, is_neox):
    x = x.clone()
    if is_neox:
        x1 = x[..., 0:HALF].clone()
...
def test_fused_k_fp4_store_matches_unfused_path(is_neox) -> None:
    ...
    k_bf16 = fused_k_indexer_norm_rope(
...
@pytest.mark.parametrize("is_neox", [False, True])
def test_fused_q_fp4_matches_eager_reference(is_neox) -> None:
    ...
    (q_fp4, q_sf), w_out = fused_q_indexer_rope_first_fp4_quant(
        q, weights_raw, weight_scale, cos_sin_cache, positions, is_neox=is_neox
    )
    ...
    ref = _rope_first_ref(q.float(), cp, sp, is_neox).to(torch.bfloat16)
